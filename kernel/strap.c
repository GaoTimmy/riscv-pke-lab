...
//   if(ret < 0)  panic( "call do_syscall to accomplish the syscall and lab1_1 here.\n" );
...
//   g_ticks++;
//   write_csr(sip, 0); // clear the "SIP" field in sip register
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
      ...
      /**
       * @para:
       * 1. current->pagetable: the pagetable of the current process.
       * 2. ROUNDDOWN(stval, PGSIZE): the virtual address that causes the page fault.
       * 3. PGSIZE: the size of the page.
       * 4. (uint64)alloc_page(): the physical address of the newly allocated page.
       * 5. prot_to_type(PROT_READ | PROT_WRITE, 1): the permission of the new page.
       *  ref: kern_vm_map(t_page_dir, (uint64)_etext, (uint64)_etext, PHYS_TOP - (uint64)_etext, prot_to_type(PROT_READ | PROT_WRITE, 0));
       */
      map_pages(current[read_tp()]->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, (uint64)alloc_page(), prot_to_type(PROT_READ | PROT_WRITE, 1));
