...
process *alloc_process()
{
  ...
  procs[i].pfiles = init_proc_file_management(1);
...
process *reset_process(process *p)
{
  ...
  // initialize files_struct
  p->pfiles = init_proc_file_management(0);
  // sprint("in alloc_proc. build proc_file_management successfully.\n");
...
int do_fork(process *parent)
{
  ...
  for (int i = 0; i < parent->total_mapped_region; i++)
  {
    ...
    case HEAP_SEGMENT:
      ...
      case DATA_SEGMENT:
      {
        uint64 size = parent->mapped_info[i].npages; // size 变量表示当前页的大小
        uint64 start_va = parent->mapped_info[i].va; // va 变量表示起始虚拟地址
        // uint64 start_pa = lookup_pa(parent->pagetable, start_va); // pa 变量表示当前页的物理地址，pagetable表示父进程的页表，lookup_pa函数用于查找虚拟地址对应的物理地址
        int permit = prot_to_type(PROT_WRITE | PROT_READ, 1); // permit 变量表示当前页的权限      
        for (int j = 0; j < size; j ++) {
          // 一页一页复制数据块
          uint64 p_pa = lookup_pa((pagetable_t)parent->pagetable, start_va + j * PGSIZE);
          void* child_get = alloc_page();
          memcpy(child_get, (void*)p_pa, PGSIZE);
          map_pages((pagetable_t)child->pagetable, start_va + j * PGSIZE, PGSIZE, (uint64)child_get, permit);
        }
        child->mapped_info[child->total_mapped_region].va = start_va;
