...
uint64 user_vm_malloc(pagetable_t page_dir, uint64 pre_size, uint64 now_size)
{
  if (pre_size < now_size)
  {
    ...
    while (pre < now_size) // allocate full physical page
    {
      char *tm = (char *)alloc_page();
      memset(tm, 0, PGSIZE * sizeof(uint8));
      map_pages(page_dir, pre_size, PGSIZE, (uint64)tm, prot_to_type(PROT_READ | PROT_WRITE, 1));
      pre += PGSIZE;
    }
    return now_size;
  }
