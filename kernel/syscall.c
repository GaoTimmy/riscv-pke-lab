...
ssize_t sys_user_exec(char* path, char* argv) {
  // 将用户空间的虚拟地址转换为物理地址
  char* path_pa = (char*)user_va_to_pa((pagetable_t)(current->pagetable), (void*)path);
  char* argv_pa = (char*)user_va_to_pa((pagetable_t)(current->pagetable), (void*)argv);

  // 计算路径和参数的长度
  int path_length = strlen(path_pa);
  int argv_length = strlen(argv_pa);

  // 分配本地缓冲区并复制内容
  char local_path[path_length + 1];
  char local_argv[argv_length + 1];
  strcpy(local_path, path_pa);
  strcpy(local_argv, argv_pa);
  local_path[path_length] = '\0';
  local_argv[argv_length] = '\0';

  // 重置当前进程状态
  reset_process(current);

  // 计算参数在用户栈中的虚拟地址，并进行对齐
  uint64 argv_va = current->trapframe->regs.sp - argv_length - 1;
  argv_va -= argv_va % 8;

  // 将参数复制到用户栈中
  uint64 argv_pa_in_stack = (uint64)user_va_to_pa(current->pagetable, (void*)argv_va);
  strcpy((char*)argv_pa_in_stack, local_argv);

  // 设置参数指针
  argv_va -= 8;
  argv_pa_in_stack = (uint64)user_va_to_pa(current->pagetable, (void*)argv_va);
  *(uint64 *)argv_pa_in_stack = argv_va + 8;

  // 更新栈指针和寄存器
  current->trapframe->regs.sp = argv_va - argv_va % 16;
  current->trapframe->regs.a0 = 1;
  current->trapframe->regs.a1 = argv_va;

  // 执行二进制代码
  int result = exec_bincode_from_host_elf(current, local_path);
